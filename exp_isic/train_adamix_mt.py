...
from exp_isic.data.dataset import ISICDataset
...
def get_data(args):
    val_set = ISICDataset(image_path=args.data_path, stage='val', image_size=args.image_size, is_augmentation=False)
    labeled_train_set = ISICDataset(image_path=args.data_path, stage='train', image_size=args.image_size, is_augmentation=True, labeled=True, percentage=args.labeled_percentage)
    unlabeled_train_set = ISICDataset(image_path=args.data_path, stage='train', image_size=args.image_size, is_augmentation=True, labeled=False, percentage=args.labeled_percentage)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            with torch.no_grad():
                ...
                pred_l_probs = torch.softmax(pred_l_logits, dim=1) # 8 4 256 256
                pred_l_pseudo = torch.argmax(pred_l_probs, dim=1) # 8 256 256

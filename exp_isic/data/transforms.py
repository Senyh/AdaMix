...
class Resize(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        """
        return F.resize(img, self.size, self.interpolation), F.resize(label, self.size, InterpolationMode.NEAREST)
...
class RandomPerspective(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        if torch.rand(1) < self.p:
            ...
            return F.perspective(img, startpoints, endpoints, self.interpolation, fill), F.perspective(label, startpoints, endpoints, InterpolationMode.NEAREST, fill)
        return img, label
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        return F.resized_crop(img, i, j, h, w, self.size, self.interpolation), F.resized_crop(label, i, j, h, w, self.size, InterpolationMode.NEAREST)
...
class RandomRotation(torch.nn.Module):
    ...
    def __init__(self, degrees, interpolation=InterpolationMode.NEAREST, expand=False, center=None, fill=0, resample=None, p=0.5):
        super().__init__()
        self.p = p
    ...
    def forward(self, img, label):
        ...
        if torch.rand(1).item() < self.p:
            return F.rotate(img, angle, self.resample, self.expand, self.center, fill), F.rotate(label, angle, self.resample, self.expand, self.center, fill)
        return img, label
...
class RandomErasing(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        if torch.rand(1) < self.p:
            ...
            return F.erase(img, x, y, h, w, v, self.inplace), F.erase(label, x, y, h, w, v, self.inplace)
        return img, label
...
class GaussianBlur(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        return F.gaussian_blur(img, self.kernel_size, [sigma, sigma]), label
...
class RandomPosterize(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        """
        if torch.rand(1).item() < self.p:
            return F.posterize(img, self.bits), label
        return img, label
...
class RandomSolarize(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        """
        if torch.rand(1).item() < self.p:
            return F.solarize(img, self.threshold), label
        return img, label
...
class RandomAdjustSharpness(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        """
        if torch.rand(1).item() < self.p:
            return F.adjust_sharpness(img, self.sharpness_factor), label
        return img, label
...
class RandomAutocontrast(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        """
        if torch.rand(1).item() < self.p:
            return F.autocontrast(img), label
        return img, label

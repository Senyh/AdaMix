...
from wheels.adamix_utils import AdaMix3D as AdaMix
...
def get_args(known=False):
    ...
    parser.add_argument('--project', type=str, default=os.path.dirname(os.path.realpath(__file__)) + '/runs/AdaMix_MT', help='project path for saving results')
    ...
    parser.add_argument('--patch_size', type=int, default=4, help='the size of patches for adamix')
...
def get_data(args):
    ...
    labeled_train_set = LADataset(image_path=args.data_path, stage='train', image_size=args.image_size, is_augmentation=True, labeled=True, percentage=args.labeled_percentage)
    unlabeled_train_set = LADataset(image_path=args.data_path, stage='train', image_size=args.image_size, is_augmentation=True, labeled=False, percentage=args.labeled_percentage)
...
def main(is_debug=False):
    ...
    teacher = networks.__dict__[args.backbone](in_channels=args.in_channels, out_channels=args.num_classes).to(device)
    ...
    criterion_s = DSCLossH(num_classes=args.num_classes, device=device, is_3d=True, inter_weight=1.)
    criterion_u = DSCLossH(num_classes=args.num_classes, device=device, is_3d=True, inter_weight=1.)

    optimizer = optim.AdamW(student.parameters(), lr=args.learning_rate, betas=(0.9, 0.999))

    # Init AdaMix
    adamix_l = AdaMix(total_steps=total_iters // 4, num_classes=args.num_classes, image_size=args.image_size, patch_size=args.patch_size, topk=args.topk, mode='hard', p=0.5)
    adamix_u = AdaMix(total_steps=total_iters // 4, num_classes=args.num_classes, image_size=args.image_size, patch_size=args.patch_size, topk=args.topk, mode='hard', p=0.5)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            optimizer.zero_grad()
            with torch.no_grad():
                pred_a = teacher(torch.cat([laimage, uaimage]))
                pred_la_logits, pred_ua_logits = pred_a['out'].chunk(2)
                pred_la_probs = torch.softmax(pred_la_logits, dim=1)
                pred_la_pseudo = torch.argmax(pred_la_probs, dim=1).clone()
                pred_la_conf = pred_la_probs.max(dim=1)[0].clone()
                pred_ua_probs = torch.softmax(pred_ua_logits, dim=1)
                pred_ua_pseudo = torch.argmax(pred_ua_probs, dim=1).clone()
                pred_ua_conf = pred_ua_probs.max(dim=1)[0].clone()

                pred_lu = teacher(torch.cat([image, uimage]))
                pred_l_logits, pred_u_logits = pred_lu['out'].chunk(2)
                pred_l_probs = torch.softmax(pred_l_logits, dim=1) 
                pred_l_pseudo = torch.argmax(pred_l_probs, dim=1) 
                pred_l_conf = pred_l_probs.max(dim=1)[0]
                pred_u_probs = torch.softmax(pred_u_logits, dim=1) 
                pred_u_pseudo = torch.argmax(pred_u_probs, dim=1) 
                pred_u_conf = pred_u_probs.max(dim=1)[0]

                pred_uA1_logits = student(uimageA1)['out']
            
            # AdaMix
            imageA1, label, pred_l_conf = adamix_l(oimage=imageA1, aimage=laimageA1, olabel=label.squeeze(1).long(), alabel=lalabel.squeeze(1).long(), oconf=pred_l_conf, aconf=pred_la_conf, prediction=pred_l_logits, cur_step=idx + len(pbar) * (epoch-1))
            uimageA1, pred_u_pseudo, pred_u_conf = adamix_u(oimage=uimageA1, aimage=uaimageA1, olabel=pred_u_pseudo, alabel=pred_ua_pseudo, oconf=pred_u_conf, aconf=pred_ua_conf, prediction=pred_uA1_logits, cur_step=idx + len(pbar) * (epoch-1))
            
            pred_l_logits = student(imageA1)['out']
            pred_uA1_logits = student(uimageA1)['out']

            # supervised path
            loss_s = criterion_s(pred_l_logits, label.long())
            # unsupervised path
            loss_u = criterion_u(pred_uA1_logits, pred_u_pseudo.detach(), pixel_mask=(pred_u_conf >= conf_threshold).float())
            ...
            included_ratio = (pred_u_conf >= conf_threshold).sum() / pred_u_conf.numel()
        ...
        if np.mean(epoch_metrics.val_loss) <= best_loss:
            ...
            torch.save(student.state_dict(), save_path + 'best.pth'.format(best_epoch))

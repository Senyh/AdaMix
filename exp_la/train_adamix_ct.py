    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            with torch.no_grad():
                ...
                s1_pred_l_probs = torch.softmax(s1_pred_l_logits, dim=1) 
                s1_pred_l_pseudo = torch.argmax(s1_pred_l_probs, dim=1) 
                s1_pred_l_conf = s1_pred_l_probs.max(dim=1)[0]
                s1_pred_u_probs = torch.softmax(s1_pred_u_logits, dim=1) 
                s1_pred_u_pseudo = torch.argmax(s1_pred_u_probs, dim=1) 
                ...
                s2_pred_l_probs = torch.softmax(s2_pred_l_logits, dim=1) 
                s2_pred_l_pseudo = torch.argmax(s2_pred_l_probs, dim=1) 
                s2_pred_l_conf = s2_pred_l_probs.max(dim=1)[0]
                s2_pred_u_probs = torch.softmax(s2_pred_u_logits, dim=1) 
                s2_pred_u_pseudo = torch.argmax(s2_pred_u_probs, dim=1)

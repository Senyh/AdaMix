...
def main(is_debug=False):
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            optimizer.zero_grad()
            with torch.no_grad():
                pred_a = student(torch.cat([laimage, uaimage]))
                ...
                pred_lu = student(torch.cat([image, uimage]))
            ...
            pred_lA1_logits = student(imageA1)['out']
            pred_uA1_logits = student(uimageA1)['out']

            # supervised path
            loss_s = criterion_s(pred_lA1_logits, label.long())
            # unsupervised path

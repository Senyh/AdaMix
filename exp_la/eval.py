...
def test_single_case(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    if add_pad:
        image = np.pad(image, [(dl_pad, dr_pad), (hl_pad,hr_pad), (wl_pad,wr_pad)], mode='constant', constant_values=0)
    ...
    for z in range(0, sz):
        ...
        for y in range(0, sy):
            ...
            for x in range(0, sx):
                ...
                y = y.cpu().data.numpy()
                y = y[0,:,:,:,:]
                score_map[:, zs:zs+patch_size[0], ys:ys+patch_size[1], xs:xs+patch_size[2]] \
                  = score_map[:, zs:zs+patch_size[0], ys:ys+patch_size[1], xs:xs+patch_size[2]] + y
                cnt[zs:zs+patch_size[0], ys:ys+patch_size[1], xs:xs+patch_size[2]] \
...
def test():
    ...
    for case in tqdm(image_list):
        ...
        image = h5f['image'][:].transpose(2, 1, 0)  # W H D -> D H W
        label = h5f['label'][:].transpose(2, 1, 0)  # W H D -> D H W
